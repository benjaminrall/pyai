...
class MaxPooling2D(Layer):
    ...
    def build(self, input_shape: tuple) -> tuple:
        ...
        pool_rows = (input_shape[0] - self.pool_size[0]) // self.strides[0] + 1
        pool_cols = (input_shape[1] - self.pool_size[1]) // self.strides[1] + 1

        self.output_shape = (pool_rows, pool_cols) + input_shape[2:]
        self.view_shape = self.output_shape[:2] + self.pool_size + input_shape[2:]

        self.built = True
        return self.output_shape

    def forward(self, input: np.ndarray) -> np.ndarray: # Input: (batches, rows, cols, channels)
